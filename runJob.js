...
const templateName = 'Information';

module.exports = {
    handle_job_run: async function(tower_address, port, orgenization, username, password, limit, extra_vars_json, tags) {
        ...
        while (["running", "pending"].indexOf(run_output.status) > -1) {
            run_output = await get_job_output(tower_address, port, username, password, job_url);
        }
...
function get_job_output(tower_address, port, username, password, url) {
    return new Promise(function(resolve, reject) {  
        let options = {
            host: tower_address,
            path: `${url}stdout/`,
